export function LiveMatchesIcon({ color = "currentColor" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      fill={color}
      width="20px"
      height="20px"
      viewBox="0 0 24 24"
      id="Flat"
    >
      <g id="Stream_On" data-name="Stream On">
        <g>
          <path
            d="M6.26,19.089A9.625,9.625,0,0,1,6.234,4.911C6.709,4.475,6,3.769,5.527,4.2A10.516,10.516,0,0,0,5.553,19.8c.475.433,1.184-.273.707-.707Z"
            fill={color}
          />
          <path
            d="M8.84,15.706a5.024,5.024,0,0,1-.014-7.412c.474-.437-.234-1.143-.707-.707a6.028,6.028,0,0,0,.014,8.826c.474.434,1.183-.272.707-.707Z"
            fill={color}
          />
          <circle cx="12" cy="12" r="1.244" fill={color} />
          <path
            d="M17.74,4.911a9.625,9.625,0,0,1,.026,14.178c-.475.436.234,1.142.707.707A10.516,10.516,0,0,0,18.447,4.2c-.475-.433-1.184.273-.707.707Z"
            fill={color}
          />
          <path
            d="M15.16,8.294a5.024,5.024,0,0,1,.014,7.412c-.474.437.234,1.143.707.707a6.028,6.028,0,0,0-.014-8.826c-.474-.434-1.183.272-.707.707Z"
            fill={color}
          />
        </g>
      </g>
    </svg>
  );
}

export function CalendarIcon({ color = "currentColor" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20px"
      height="20px"
      viewBox="0 0 24 24"
      fill="none"
    >
      <path
        d="M14 22H10C6.22876 22 4.34315 22 3.17157 20.8284C2 19.6569 2 17.7712 2 14V12C2 8.22876 2 6.34315 3.17157 5.17157C4 4 6.22876 4 10 4H14C17.7712 4 19.6569 4 20.8284 5.17157C22 8.22876 22 12V14C22 17.7712 20.8284 20.8284C20.1752 21.4816 19.3001 21.7706 18 21.8985"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      <path d="M7 4V2.5" stroke={color} strokeWidth="1.5" strokeLinecap="round" />
      <path d="M17 4V2.5" stroke={color} strokeWidth="1.5" strokeLinecap="round" />
      <path
        d="M21.5 9H16.625H10.75M2 9H5.875"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      <path
        d="M18 17C18 17.5523 17.5523 18 17 18C16.4477 18 16 17.5523 16 17C16 16.4477 16.4477 16 17 16C17.5523 16 18 16.4477 18 17Z"
        fill={color}
      />
      <path
        d="M18 13C18 13.5523 17.5523 14 17 14C16.4477 14 16 13.5523 16 13C16 12.4477 16 12 17 12C17.5523 12 18 13Z"
        fill={color}
      />
      <path
        d="M13 17C13 17.5523 12.5523 18 12 18C11.4477 18 11 17.5523 11 17C11 16.4477 11.4477 17 12 17C12.5523 17 13 17 13 17Z"
        fill={color}
      />
      <path
        d="M13 13C13 13.5523 12.5523 14 12 14C11.4477 14 11 13 11 13C11 12.4477 11.4477 12 12 12C12.5523 12 13 13Z"
        fill={color}
      />
      <path
        d="M8 17C8 17.5523 7.55228 18 7 18C6.44772 18 6 17.5523 6 17C6 16.4477 6.44772 16 7 16C7.55228 16 8 16 8 17Z"
        fill={color}
      />
      <path
        d="M8 13C8 13.5523 7.55228 14 7 14C6.44772 14 6 13 6 13C6 12.4477 6.44772 12 7 12C7.55228 12 8 12 8 13Z"
        fill={color}
      />
    </svg>
  );
}

export function TopMatchIcon({ color = "currentColor" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20px"
      height="20px"
      viewBox="0 0 24 24"
      fill="none"
    >
      <path
        d="M16.5582 15.0001C16.835 14.3566 17 13.5859 17 12.6667C17 10.0558 15.4593 8.16231 14.0008 7.03166C13.2525 6.45159 12.2941 7.05323 12.2941 8.00001C12.2941 8.64273 12.0054 9.71299 11.4205 10.5011C10.7814 11.3622 9.95616 10.5021 9.89841 9.43131C9.87201 8.94193 9.36421 8.76864 8.95593 9.03976C8.06292 9.63276 7 10.8109 7 12.6667C7 16.9333 10.1111 18 11.6667 18C12.474 18 13.648 17.78 14.6891 17.1403"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
      <path
        d="M22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C21.5093 4.43821 21.8356 5.80655 21.9449 8"
        stroke={color}
        strokeWidth="1.5"
        strokeLinecap="round"
      />
    </svg>
  );
}

export function TopScorerIcon({ color = "currentColor" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20px"
      height="20px"
      viewBox="0 0 24 24"
      fill={color}
    >
      <path
        d="M21.92,5.62A1,1,0,0,0,21,5H19V3a1,1,0,0,0-.62-.92,1,1,0,0,0-1.09.21l-3,3A1,1,0,0,0,14,6V8.59l-2.21,2.2a1,1,0,0,0,0,1.42,1,1,0,0,0,1.42,0L15.41,10H18a1,1,0,0,0,.71-.29l3-3A1,1,0,0,0,21.92,5.62Z"
        fill={color}
      />
      <path
        d="M12,22A10,10,0,0,1,12,2h.42a1,1,0,1,1-.18,2H12a8,8,0,1,0,8,8,1.93,1.93,0,0,0,0-.24,1,1,0,0,1,.91-1.09,1,1,0,0,1,1.09.91c0,.14,0,.28,0,.42A10,10,0,0,1,12,22Zm5.88-8.8a1,1,0,0,0-2-.4A4,4,0,1,1,11.2,8.08a1,1,0,1,0-.4-2,6,6,0,1,0,7.08,7.08Z"
        fill={color}
      />
    </svg>
  );
}

export function ArrowRightIcon({ color = "currentColor" }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="20px"
      height="20px"
      viewBox="0 0 24 24"
      fill="none"
    >
      <path
        d="M6 12H18M18 12L13 7M18 12L13 17"
        stroke={color}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}